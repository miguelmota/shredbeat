...
class ShredometerService extends EventEmitter {
  constructor() {
    ...
    var lastShredProgress = null
    var lastRate = null
    var lastAmplitude = null

    setInterval(() => {
      this.currentRate += (this.rate *  this.sensitivity * this.direction)
      this.shredProgress = this.progressScale(this.currentRate)
      ...
      if (lastShredProgress !== this.shredProgress) {
        this.emit('ShredProgress', this.shredProgress)
      }
      if (lastRate !== this.currentRate) {
        this.emit('ShredRate', this.currentRate)
      }
      if (lastAmplitude !== this.amplitude) {
        this.emit('AmplitudeRate', this.amplitude)
      }

      lastShredProgress = this.shredProgress
      lastRate = this.currentRate
      lastAmplitude = this.amplitude
